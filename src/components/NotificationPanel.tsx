    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border z-50 max-h-96 overflow-y-auto">
            <div className="sticky top-0 bg-white border-b rounded-t-2xl px-4 py-3 flex items-center justify-between">
              ...
              <button onClick={() => setOpen(false)} className="cursor-pointer p-1 rounded-lg hover:bg-gray-100">
                <X size={16} className="text-gray-400" />
              </button>
            </div>
            {notifications.length === 0 ? (
              <div className="p-8 text-center text-gray-400 text-sm">{t('no_notifications')}</div>
            ) : (
